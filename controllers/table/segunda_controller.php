<?php
  ...
  if(mysqli_num_rows($result) > 0 && $_SESSION['tipo_de_acesso'] == 1) {
    ...
    while( $row = mysqli_fetch_assoc($result) ) {
      echo "<tr class='align'><td>" . $row['id'] . "</td><td>" . $row['nome'] . "</td><td>" . $row['turno'] . "</td><td>" . $row['disciplina'] . "</td><td>" . $row['professor'] . "</td><td>" . $row['sala'] . "</td><td>" . $row['espaco'] . "</td></tr>";
    }
  }else {
